...
import { BankStore, AccountType, BankAccount } from "@/lib/bankStore";
import { MicroLearningWrapper } from "@/components/module/MicroLearningWrapper";
import { Landmark, ArrowLeft, Hash, Edit2, AlertTriangle, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export default function AddBankAccount() {
    ...
    const handleCheckPrefix = () => {
        ...
        const { exact, possible } = BankStore.checkDuplicate(bankName, accountType, last4, nickname);
        if (exact) {
    ...
    const handleSave = () => {
        ...
        BankStore.addAccount({
            ...
            status,
        } as Omit<BankAccount, "id" | "createdAt" | "updatedAt" | "auditTrail"> & { asOfDate: string });
            ...
            <div className="bg-black text-white min-h-screen px-6 py-6 pb-24 font-sans animate-fade-in relative">
                ...
                <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/90 to-transparent">
                    <button
                        ...
                        onClick={duplicateWarning && !error ? () => { } : handleSave}
                    ...
                    >
                        {duplicateWarning && !error ? "Resolve Warning Above" : "Save Account"}
                    </button>
                </div>
            </div>
        </MicroLearningWrapper>
