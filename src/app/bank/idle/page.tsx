...
export default function IdleMoneyEngine() {
    ...
    useEffect(() => {
        setAccounts(BankStore.getAccounts().filter(a => a.status === "active"));
        const baseThreshold = BankStore.getSettings().idleThresholdAmount;
        setThreshold(baseThreshold.toString());
        setOutflow(BankStore.getCashFlowMetrics().outflow);
        setTargetMonths(BankStore.getSettings().emergencyFundTargetMonths);
    }, []);
    ...
    const handleSave = () => {
        ...
        BankStore.updateSettings({ idleThresholdAmount: activeThreshold });
        router.push("/bank");
    };
                                        ...
                                        <div className="flex justify-between items-start mb-2 relative z-10">
                                            <div>
                                                ...
                                                <p className="text-[11px] text-white/40 mt-0.5 uppercase tracking-wider">
                                                    {BankStore.maskAccountNumber(acc.accountLast4)}
                                                </p>
                                            </div>
