...
import { BankStore, CashWallet } from "@/lib/bankStore";
import { ArrowLeft, Wallet, ShieldAlert, Coins, CheckCircle2 } from "lucide-react";

export default function CashWalletPage() {
    const router = useRouter();

    const [cash, setCash] = useState<CashWallet>({ cashInHand: 0, emergencyCash: 0, pettyCash: 0, updatedAt: "" });
    const [error, setError] = useState("");
    const [success, setSuccess] = useState(false);

    // Temporary string state for inputs to allow empty fields during typing
    const [inHandStr, setInHandStr] = useState("");
    const [emergencyStr, setEmergencyStr] = useState("");
    const [pettyStr, setPettyStr] = useState("");

    useEffect(() => {
        const data = BankStore.getCashWallet();
        setCash(data);
        if (data.cashInHand > 0) setInHandStr(data.cashInHand.toString());
        if (data.emergencyCash > 0) setEmergencyStr(data.emergencyCash.toString());
        if (data.pettyCash > 0) setPettyStr(data.pettyCash.toString());
    }, []);
