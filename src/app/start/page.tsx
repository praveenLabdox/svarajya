...
import { MessageSquareOff, Landmark, ShieldCheck, Mail, Lock, ArrowRight, Loader2, User, Check, X } from "lucide-react";
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (mode === "signup") {
                ...
                setError("");
                setMode("login");
                setMsg("Reset link sent to your email. Please check your inbox.");
                return;
            } else {
                const { error: signInError } = await supabase.auth.signInWithPassword({
                    email: trimmedEmail,
                ...
                <div className="flex items-center justify-center gap-5">
                    {TRUST_ICONS.map((t, i) => (
                        <div key={i} className="flex flex-col items-center gap-1.5">
                            ...
                            <span className="text-[9px] text-[#f2faf5] text-center leading-tight max-w-[56px]">
                                {t.label}
                            </span>
                        </div>
                    ))}
